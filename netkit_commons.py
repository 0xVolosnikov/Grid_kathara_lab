...
def create_commands(machines, links, options, metadata, path, execbash=False, no_machines_tmp=False, network_counter=0):
    ...
    if PLATFORM != WINDOWS:
        ...
        create_machine_template = docker + ' run --volume="' + os.path.expanduser('Kathara') +'":/hosthome -tid --privileged=true --name ' + prefix + '{machine_name} --hostname={machine_name} --network=' + prefix + '{first_link} {machine_options} {image_name}'
